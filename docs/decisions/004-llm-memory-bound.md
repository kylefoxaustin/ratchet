# ADR 004: LLM workload modeled as memory-BW-bound

**Status:** Accepted
**Date:** 2026-04 (originally rescue-bird ADR 008)
...
The `llm_demand` calculator (in `ratchet.engine.demand`) computes:
...
- **Skip LLM entirely.** Rejected: on-device reasoning ("is this a
  person or a backpack?", voice command interpretation, agentic tool
  use) is a real feature ask for many edge sizer use cases. Pretending
  it doesn't exist produces an under-spec'd chip.
