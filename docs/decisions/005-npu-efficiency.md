# ADR 005: NPU efficiency factor of 0.55 default

**Status:** Accepted (provisional — needs measured calibration)
**Date:** 2026-04 (originally rescue-bird ADR 009)
...
Default `npu.efficiency_factor` is 0.55 in the engine's `eff_tops`
helper. Sites can override per-profile and via sliders (typical range
0.30-0.90).

Rationale: 0.55 is the median of measured BF16 transformer workloads on
modern edge NPUs that the project author trusts. Lower than 0.7 because
target workloads are transformer-heavy (SAM/TAM, BEVFusion, LLM).
Higher than 0.4 because the candidate silicon is in the post-2024
generation with mature compilers.
...
- **0.7 default**: too optimistic for transformer-heavy classes. Would
  produce reports that pass at default but fail in practice — exactly
...
- **This is the most-likely-to-be-wrong constant in the engine.** It's
...
- For competitive analysis, the slider lets the user show "even at
  0.4 efficiency this chip still meets the KPIs" — a stronger claim
  than "at 0.55 it works."

## Worth knowing

Three workloads to measure first, in priority order for a drone-class
sizer:
