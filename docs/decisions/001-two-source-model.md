...
**Status:** Accepted
**Date:** 2026-04 (originally rescue-bird ADR 001)
**Decision drivers:** speed of iteration, defensibility of numbers,
calibration discipline.

## Context

A sizing project has two inseparable goals:
...
A pure-measurement approach (run the instrumented stack, read the Parquet,
write the report) gives defensible numbers but takes 5-15 minutes per data
point and requires functioning real models for every subsystem. A pure-
analytical approach (math from spec sheets) iterates fast but isn't credible
to anyone who hasn't seen it line up against reality.

## Decision

Both, side by side. Every subsystem produces:
- **Measured** numbers from the instrumented run (Parquet records via probes)
...
- The instrumentation framework has to work even when models are stubs.
  The point is to validate the *plumbing* before the real models exist.
- Reports must clearly label which numbers came from where. The "delta
  column" is the most important UX in the report.

## Worth knowing

The most-likely-to-be-wrong constants are the NPU efficiency factor (see
ADR 005) and the per-stage ISP/DSP timing tables in each consuming site.
Calibrating those constants against measurements is high-leverage.
