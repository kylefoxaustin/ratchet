...
Every probe in the system emits records with this shape. They land in Parquet
files under ./runs/<timestamp>/ and are aggregated by site-specific report
generators (e.g. nightjar's ``soc_partition_report.py``).

Design philosophy: capture what matters for SoC partitioning, not what's easy
to collect. The fields below are chosen so that, given enough records across
enough mission scenarios, you can answer:
  ...
  - Per phase: duty cycle on each block
  - Per precision (BF16 vs FP16 vs INT8): accuracy delta and compute delta

The schema is deliberately wide and sparse. Probes fill in only the fields
they can measure cheaply.

V1 compromise — drone-shaped Optional fields
─────────────────────────────────────────────
The record carries domain-specific fields that originated in the drone use
case: ``radar_*``, ``isp_*``, ``dsp_*``, ``g2g_*``, ``pos_*``, ``target_locked``,
``llm_*``, ``encode_*``, ``link_*``. They are all ``Optional`` and consuming
sites simply leave unused fields as ``None``.

This keeps the Parquet schema unified across sites in v1 (drone, video,
agentic-AI sizers all read/write the same table). The cost is that every
site's records nominally carry columns it doesn't fill. Acceptable for now.
Revisit when a second site (keyhole or skippy) is in flight and we know what
domain-specific fields they actually want; at that point either widen further
or split into a base record + per-site extension schemas.

Site-specific subsystem and phase string constants (e.g.
``SUBSYSTEM_RADAR``, ``PHASE_RTH``) live in the consuming site's package,
not here. ratchet's record schema does not enumerate valid subsystem names.
"""
...
class WorkloadRecord:
    ...
    Records are append-only. Probes emit one per work unit (e.g. one
    inference, one encoded frame, one MAVLink batch). The aggregator buckets
    and rolls up later.
    """
    ...
    # ──── What was measured ────
    subsystem: str = ""               # site-specific label (e.g. "perception", "encoder")
    operation: str = ""               # free-form, e.g. "edgetam_decoder", "h265_encode"
    phase: str = "idle"               # site-specific phase label
    ...
    # ──── Encode-specific (drone, video sizer) ────
    ...
    # ──── Comms-specific (drone, video sizer) ────
    ...
    # ──── Glass-to-glass latency (drone, video sizer) ────
    # Emitted by the dedicated end-to-end probe that timestamps a frame at
    # every stage. Only the final stage record fills all fields.
    ...
    # ──── DSP-specific (drone — Cadence Vision Q-class) ────
