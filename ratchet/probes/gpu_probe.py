...
If ``pynvml`` is not installed (ratchet's ``[gpu]`` extra), the probe degrades
to a no-op rather than failing at import time.
"""
...
class GpuProbe:
    ...
    def __init__(
        ...
        gpu_index: int = 0,
        phase_provider=None,   # callable returning current phase string
    ) -> None:
        self.writer = writer
        self.run_id = run_id
